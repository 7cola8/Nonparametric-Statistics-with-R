...
- **身高体重相关性分析与顾客购买行为检验.pdf**  
本实验报告通过两个实际案例，系统性地应用了多种非参数统计方法，包括：Spearman 秩相关检验；Kendall τ 检验；Theil 回归分析；广义符号检验；Wilcoxon 符号秩检验。

## 🔧 使用工具

- 编程语言：R  
- 使用方法：通过 R 语言编写检验函数与调用运行，实现了检验统计量的计算与 p 值判断，适合具备基本 R 编程能力的用户阅读与复现。
- R base 包内置函数：`wilcox.test()`、`fisher.test()`、`t.test()`、 `stats`、`mblm`、自定义函数 `sign.text` 实现广义符号检验。
...
10. **案例一：身高与体重的相关性分析**
- 使用 Spearman 和 Kendall 两种秩相关方法，检验身高与体重的正相关性；
- 分别构建了一般线性回归模型与 Theil 中位数回归模型，并进行了对比；
- 使用 R 语言完成数据分析与可视化。
11. **案例二：顾客购物件数是否为 10 件的检验**
- 采用广义符号检验和 Wilcoxon 符号秩检验判断顾客平均购物件数是否小于 10；
- 分析两种方法的适用性与结论可靠性。
